...
#Remove shot_id and shot_made_flag and then cluster
ShotId<-rawDataPreped$shot_id
shotOutcome<-rawDataPreped$shot_made_flag

clusterData<-rawDataPreped%>%
  select(-shot_id, -shot_made_flag)

library(cluster)
clusterOutput<-daisy(clusterData,metric = "Gower")
