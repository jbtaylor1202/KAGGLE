...
#Fit and examin model
logit<-glm(data = trainData, shot_made_flag ~ ., family = "binomial")
summary(logit)

logitPred<-predict(logit, testData,type = "response")
logitSubmit<-data.frame(shot_id = testShotIDs, shot_made_flag=logitPred)
write.csv(logitSubmit, file = "Model 20 - Logit1 (All Features).csv", row.names=FALSE)


logit2<-step(logit)

logit2<-glm(formula = shot_made_flag ~ combined_shot_type + minutes_remaining + 
            period + season + seconds_remaining + shot_zone_area + shot_zone_basic + 
           shot_zone_range + action_type_2 + location, family = binomial, data = trainData)

summary(logit2)

logit2Pred<-predict(logit2, testData, type = "response")
logit2Submit<-data.frame(shot_id = testShotIDs, shot_made_flag=logit2Pred)
write.csv(logit2Submit, file = "Model 21 - Logit1 (All Features STEP).csv", row.names=FALSE)
