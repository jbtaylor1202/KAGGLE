...
save(rfFit,file = "Model38.RData")

summary(rfFit)
rfFit

predClasses<-predict(rfFit,testData)
predProbs<-predict(rfFit,testData, type="prob")
testData$shot_made_flag = predProbs[,2]
submit <- data.frame(shot_id = testShotIDs, shot_made_flag=testData$shot_made_flag)

write.csv(submit, file = "Model 38 - rf caret radians.csv", row.names = FALSE)
