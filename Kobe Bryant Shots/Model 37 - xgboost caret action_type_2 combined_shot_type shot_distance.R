...
##################
rawData$loc_x_abs<-abs(rawData$loc_x)
rawData$shotAngleRadians<-acos(rawData$loc_x_abs/rawData$shot_distance)
rawData$shotAngleRadians[rawData$shotAngleRadians=='NaN']<-0
...
predClasses<-predict(xgbTreeFit,testData)
predProbs<-predict(xgbTreeFit,testData, type="prob")
testData$shot_made_flag = predProbs[,2]
submit <- data.frame(shot_id = testShotIDs, shot_made_flag=testData$shot_made_flag)
write.csv(submit, file = "Model 37 - xgboost caret action_type_2 combined_shot_type shot_distance.csv", row.names = FALSE)
